package dev.hectorolea.food.ordering.system.customer.service.messaging.publisher.kafka;

import dev.hectorolea.food.ordering.system.customer.service.domain.config.CustomerServiceConfigData;
import dev.hectorolea.food.ordering.system.customer.service.domain.event.CustomerCreatedEvent;
import dev.hectorolea.food.ordering.system.customer.service.domain.ports.output.message.publisher.CustomerMessagePublisher;
import dev.hectorolea.food.ordering.system.customer.service.messaging.mapper.CustomerMessagingDataMapper;
  ...
  public void publish(CustomerCreatedEvent customerCreatedEvent) {
    ...
    try {
      CustomerAvroModel customerAvroModel =
          customerMessagingDataMapper.paymentResponseAvroModelToPaymentResponse(
  ...
  private ListenableFutureCallback<SendResult<String, CustomerAvroModel>> getCallback(
    ...
    return new ListenableFutureCallback<>() {
      ...
      public void onFailure(Throwable throwable) {
        log.error(
            "Error while sending message {} to topic {}", message.toString(), topicName, throwable);
      }
